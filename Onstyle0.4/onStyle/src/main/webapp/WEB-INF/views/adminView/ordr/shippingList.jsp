...
function shippingUpdete(shipping){ //처리여부 업데이트될때에 ### credit-> 변수명임 나중에 내 맘대로 정할 수 있음.
	var formData = 'order_state='+shipping+"&"; 
	$('input:checkbox[name=checked]:checked').each(function() {
        formData += "order_seq_list="+$(this).val()+"&";
	});
	formData = formData.substr(0,formData.length-1);
// 	alert(formData);
	if(formData == 'order_state='+shipping){
		$("#warn").html("목록을 선택해주세요!");
	}else{
		$.ajax({
			url: "/shippingUpdete.do",
			data: formData,
			type: "post",
			success: function(){
				window.location.href="/shippingList.do";
			},
							...
							<form name="" id="" method="POST">
								<div>
									<select style="height: 37px; width:10%; padding-left:10px; float:left;">
										<option value="010" selected> 배송시작일 </option>
										<option value="010" > 주문일 </option>  
			...
			<thead></thead>
				<tr>
					<th colspan="8" style="border-bottom: 1px solid #C6C6C6; width:25%; height:30px;">
						<a href="javascript:shippingUpdete('배송완료')" class="btn btnjin btn-xs" style="margin-left:10px; float:left;"> 배송완료 처리 </a> 
						<a href="javascript:shippingUpdete('배송준비중')" class="btn btnjin btn-xs" style="margin-left:10px; float:left;"> 배송준비중 처리 </a>
				...
				<tr>
					...
					<th style="text-align: center;">주문일 / [주문번호]</th>
				...
				<c:forEach var ="ordrVO" items="${shippingsList}">
				<tr onmouseover="this.style.backgroundColor = '#EBF7FF'" onmouseout="this.style.backgroundColor = ''" style="cursor:pointer">
					...
					<td style="text-align: center;">${ordrVO.shipping_date}</td>
					<td style="text-align: center;">${ordrVO.order_dlivy_num}</td>
