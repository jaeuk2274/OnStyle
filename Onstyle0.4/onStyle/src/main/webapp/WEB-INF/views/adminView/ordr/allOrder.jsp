...
$(document).ready(function(){
	...
	$("#listSelect").change(function(){
		var value = $(this).val();
		if(value == '교환신청'){
			$("#exchange_div").html('<select id="exchange"style="height: 37px; width:20%; padding-left:10px; float:left;">'
									   +'<option value="pnme1" selected> 입금확인 </option>'
									   +'<option value="pnme2"> 상품 준비중 </option>'
									   +'<option value="pnme3"> 배송 준비중 </option>'
						   		   +'</select>');
		}
		else{
			$("#exchange_div").html('');
			...
			<thead></thead>
				<tbody>
					...
					<tr>
						...
						<td>
							<select id="listSelect"style="height: 37px; width:20%; padding-left:10px; float:left;">
								<option value="pnme1" selected> 입금확인 </option> 
								<option value="pnme2"> 상품 준비중 </option>
								<option value="pnme3"> 배송 준비중 </option>
								<option value="pnme4"> 배송중 </option>
								<option value="pnme5"> 입금전 취소 신청 </option>
								<option value="pnme6"> 입금후 취소 신청 </option>
								<option value="교환신청"> 교환 신청 </option>
								<option value="pnme8"> 반품 신청 </option>
							</select>
		...
		<div class="board view view_v2" >
			...
			<thead></thead>
				...
				<tr>
					...
					<th style="text-align: center;">총 상품구매 금액</th>
					<th style="text-align: center;">결제수단</th>
					<th style="text-align: center;">결제상태</th>
					<th style="text-align: center;">배송중</th>
					<th style="text-align: center;">배송완료</th>
					<th style="text-align: center;">취소</th>
					<th style="text-align: center;">교환</th>
					<th style="text-align: center;">반품</th>
				</tr>
				<c:forEach var ="ordrVO" items="${allOrderList}">
				<tr style="overflow:scroll; width:100%; height:200px;">
					...
					<td style="text-align: center;">${ordrVO.order_methd}</td>
