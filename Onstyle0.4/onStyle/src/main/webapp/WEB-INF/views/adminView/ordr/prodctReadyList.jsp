...
function prodctsUpdete(prodcts){ //처리여부 업데이트될때에 ### credit-> 변수명임 나중에 내 맘대로 정할 수 있음.
	var formData = 'order_state='+prodcts+"&"; 
	$('input:checkbox[name=checked]:checked').each(function() {
        formData += "order_seq_list="+$(this).val()+"&";
	});
	formData = formData.substr(0,formData.length-1);
	alert(formData);
	if(formData == 'order_state='+prodcts){
		$("#warn").html("목록을 선택해주세요!");
	}
	else{
		$.ajax({
			url: "/prodctsUpdate.do",
			data: formData,
			type: "post",
			success: function(){
				window.location.href="/prodctReadyList.do";
			},
			...
			<thead></thead>
				<tr>
					<th colspan="8" style="border-bottom: 1px solid #C6C6C6; width:25%; height:30px;">
						<a href="javascript:prodctsUpdete('배송준비중')" class="btn btnjin btn-xs" style="margin-left:10px; float:left;"> 배송준비중 처리 </a> 
						<a href="javascript:prodctsUpdete('배송보류')" class="btn btnjin btn-xs" style="margin-left:10px; float:left;"> 배송보류 처리 </a>
				...
				<c:forEach var ="ordrVO" items="${prodctsList}">
